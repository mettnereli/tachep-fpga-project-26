...
void find_clusters_stream(const tower_et_t towers[30][60],
    ...
    VITIS_LOOP_347_1: for (int eta = HALF_W; eta < 30 - HALF_W; eta++) {
        VITIS_LOOP_348_2: for (int phi = 0; phi < 60; phi++) {
#pragma HLS PIPELINE II=1
...
__attribute__((sdx_kernel("calo_trigger_em_jet_stream_ref", 0))) void calo_trigger_em_jet_stream_ref(const tower_et_t towers[30][60],
...
#pragma HLS ARRAY_PARTITION variable=top_clusters complete dim=1
 int local_num_clusters = 0;
    ...
    VITIS_LOOP_456_1: for (int i = 0; i < 8; i++) {
#pragma HLS UNROLL
 top_clusters[i].et = 0;
    ...
    VITIS_LOOP_467_2: for (int i = 0; i < NUM_CANDIDATES; i++){
#pragma HLS PIPELINE II=1
...
#pragma HLS ARRAY_PARTITION variable=top_clusters complete dim=1
 VITIS_LOOP_174_1: for (int i = 0; i < 8; ++i) {
#pragma HLS UNROLL
...
__attribute__((sdx_kernel("calo_trigger_em_jet_stream_ref", 0))) void calo_trigger_em_jet_stream_ref(const tower_et_t towers[30][60],
                                    ...
                                    int *num_em_clusters,
                                    int *num_jet_clusters) {
#line 33 "/scratch/emettner/tachep-fpga-project-26/run_calorimeter_streaming.tcl"
#pragma HLSDIRECTIVE TOP name=calo_trigger_em_jet_stream_ref
# 281 "calorimeter.cpp"
...
#pragma HLS PIPELINE II=1
 ...
 Cluster candidate;
            ...
            candidate.eta = eta;
            candidate.phi = phi;
            candidate.window_size = 3;
            candidate.iso_outer_size = 5;
            candidate.valid = false;
                                ...
                                hls::stream<Cluster> &cluster_stream)
{
    ...
    VITIS_LOOP_375_1: for (int eta = R; eta < 30 - R; eta++) {
        VITIS_LOOP_376_2: for (int phi = 0; phi < 60; phi++) {
#pragma HLS PIPELINE II=1
 ...
 Cluster candidate;
            ...
            candidate.eta = eta;
            candidate.phi = phi;
            candidate.window_size = 7;
            candidate.iso_outer_size = 7;
            candidate.valid = false;
