...
__attribute__((sdx_kernel("calo_trigger_ref", 0))) void calo_trigger_ref(const tower_et_t grid[30][60],
...
cluster_et_t window_sum(const tower_et_t towers[30][60], int eta, int phi) {
    cluster_et_t sum = 0;
    int half_W = W / 2;
    VITIS_LOOP_145_1: for (int d_eta = -half_W; d_eta <= half_W; ++d_eta) {
        VITIS_LOOP_146_2: for (int d_phi = -half_W; d_phi <= half_W; ++d_phi) {
...
bool is_local_maximum(const tower_et_t towers[30][60], int eta, int phi) {
    ...
    VITIS_LOOP_165_1: for (int d_eta = -half_W; d_eta <= half_W; ++d_eta) {
        VITIS_LOOP_166_2: for (int d_phi = -half_W; d_phi <= half_W; ++d_phi) {
...
void find_clusters(const tower_et_t towers[30][60],
    ...
    VITIS_LOOP_218_1: for (int eta = HALF_W; eta < 30 - HALF_W; eta++) {
        VITIS_LOOP_219_2: for (int phi = HALF_W; phi < 60 - HALF_W; phi++) {
...
void select_top_n(const Cluster clusters[(30 * 60)],
    ...
    VITIS_LOOP_108_1: for (int i = 0; i < 8; i++) {
        top_clusters[i].et = 0;
    ...
    VITIS_LOOP_118_2: for (int i = 0; i < (30 * 60); ++i) {
 if (i >= num_clusters) {
  continue;
 }
        if (!clusters[i].valid) {
  continue;
        ...
        VITIS_LOOP_128_3: for (int j = 0; j < 8; j++) {
            if (!top_clusters[j].valid || cluster_is_better(candidate, top_clusters[j])) {

                VITIS_LOOP_131_4: for (int k = 8 - 1; k > j; k--) {
...
ht_t compute_ht(const tower_et_t towers[30][60]) {
    ht_t ht = 0;
    VITIS_LOOP_145_1: for (int eta = 0; eta < 30; eta++) {
        VITIS_LOOP_146_2: for (int phi = 0; phi < 60; phi++) {
...
void build_trigger_objects(const Cluster top_clusters[8],
                          TriggerObject trigger_objects[8]) {
    VITIS_LOOP_156_1: for (int i = 0; i < 8; ++i) {
...
__attribute__((sdx_kernel("calo_trigger_ref", 0))) void calo_trigger_ref(const tower_et_t grid[30][60],
                      ...
                      int *num_clusters)
{
#line 33 "/scratch/emettner/tachep-fpga-project-26/run_calorimeter.tcl"
#pragma HLSDIRECTIVE TOP name=calo_trigger_ref
# 194 "calorimeter.cpp"
