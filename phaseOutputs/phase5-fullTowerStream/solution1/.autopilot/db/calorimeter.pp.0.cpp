...
void calo_trigger_ref(const tower_et_t grid[30][60],
                      tower_et_t seed_threshold,
                      cluster_et_t cluster_threshold,
                      TriggerObject objects[8],
                      ht_t *ht,
                      int *num_clusters);



void find_clusters_3x3(const tower_et_t towers[30][60],
                    tower_et_t seed_threshold,
                    cluster_et_t cluster_threshold,
                    Cluster clusters[(30 * 60)],
                    int &num_clusters);

void find_clusters_3x3_iso5(const tower_et_t towers[30][60],
                           tower_et_t seed_threshold,
                           cluster_et_t cluster_threshold,
                           Cluster clusters[(30 * 60)],
                           int &num_clusters);

void find_clusters_3x3_iso7(const tower_et_t towers[30][60],
                           tower_et_t seed_threshold,
                           cluster_et_t cluster_threshold,
                           Cluster clusters[(30 * 60)],
                           int &num_clusters);

void find_clusters_5x5(const tower_et_t towers[30][60],
                    tower_et_t seed_threshold,
                    cluster_et_t cluster_threshold,
                    Cluster clusters[(30 * 60)],
                    int &num_clusters);

void find_clusters_5x5_iso7(const tower_et_t towers[30][60],
                           tower_et_t seed_threshold,
                           cluster_et_t cluster_threshold,
                           Cluster clusters[(30 * 60)],
                           int &num_clusters);

void find_clusters_7x7(const tower_et_t towers[30][60],
                    tower_et_t seed_threshold,
                    cluster_et_t cluster_threshold,
                    Cluster clusters[(30 * 60)],
                    int &num_clusters);
...
void select_top_n(const Cluster clusters[(30 * 60)],
                  int num_clusters,
...
ht_t compute_ht(const tower_et_t towers[30][60]);

void build_trigger_objects(const Cluster top_clusters[8],
                          TriggerObject trigger_objects[8]);

void run_reference_trigger_3x3_iso5(const tower_et_t towers[30][60],
                            tower_et_t seed_threshold,
                            cluster_et_t cluster_threshold,
                            Cluster clusters[(30 * 60)],
                            int &num_clusters,
...
inline void clear_top_clusters(Cluster top_clusters[8])
{
#pragma HLS INLINE

 VITIS_LOOP_190_1: for (int i = 0; i < 8; i++) {
#pragma HLS UNROLL
...
template<int W>
cluster_et_t window_sum(const tower_et_t towers[30][60], int eta, int phi) {
#pragma HLS INLINE
...
template<int W>
cluster_et_t local_window_sum_7x7(const tower_et_t window[7][7]) {
#pragma HLS INLINE

 cluster_et_t sum = 0;
    const int OFFSET = (7 - W) / 2;
    VITIS_LOOP_274_1: for (int i = 0; i < W; i++) {
#pragma HLS UNROLL
 VITIS_LOOP_276_2: for (int j = 0; j < W; j++) {
#pragma HLS UNROLL
...
template<int W>
bool is_local_maximum(const tower_et_t towers[30][60], int eta, int phi) {
#pragma HLS INLINE
...
template<int W>
bool local_is_maximum_7x7(const tower_et_t window[7][7]) {
#pragma HLS INLINE
 const int OFFSET = (7 - W) / 2;
    const int CENTER = (7 / 2);

    tower_et_t center_et = window[CENTER][CENTER];
    bool is_max = true;


    VITIS_LOOP_332_1: for (int i = 0; i < W; i++) {
#pragma HLS UNROLL
 VITIS_LOOP_334_2: for (int j = 0; j < W; j++) {
#pragma HLS UNROLL
...
cluster_et_t isolation_sum(const tower_et_t towers[30][60], int eta, int phi) {
#pragma HLS INLINE
...
#pragma HLS INLINE
 ...
 shape_calc_t iso_lhs = (shape_calc_t) 6 * iso5_et;
    ...
    shape_calc_t ring_lhs = (shape_calc_t) 6 * ring7_et;
...
#pragma HLS INLINE
 ...
 Cluster candidate;
    init_cluster(candidate, center_eta, center_phi, 3, 5);

    tower_et_t seed_et = window[(7 / 2)][(7 / 2)];

    if (seed_et >= seed_threshold &&
        local_is_maximum_7x7<3>(window)) {
        ...
        if (core3_et >= cluster_threshold && is_em_like(core3_et, iso5_et, ring7_et)) {
            set_valid_cluster(candidate, core3_et, iso5_et, center_eta, center_phi, 3, 5);
        }
    }
...
#pragma HLS INLINE
 ...
 Cluster candidate;
    init_cluster(candidate, center_eta, center_phi, 7, 7);

    tower_et_t seed_et = window[(7 / 2)][(7 / 2)];

    if (seed_et >= seed_threshold &&
        local_is_maximum_7x7<7>(window)) {
        ...
        if (sum7_et >= cluster_threshold && is_jet_like(core3_et, sum7_et)) {
            set_valid_cluster(candidate, sum7_et, ring7_et, center_eta, center_phi, 7, 7);
        }
    }
...
void find_clusters(const tower_et_t towers[30][60],
                    tower_et_t seed_threshold,
                    cluster_et_t cluster_threshold,
                    Cluster clusters[(30 * 60)],
                    int &num_clusters) {
    ...
    FIND_ETA: for (int eta = HALF_W; eta < 30 - HALF_W; eta++) {
        FIND_PHI: for (int phi = 0; phi < 60; phi++) {
...
template<int cluster_W, int iso_W>
void find_clusters_stream(const tower_et_t towers[30][60],
                    tower_et_t seed_threshold,
                    cluster_et_t cluster_threshold,
    ...
    VITIS_LOOP_531_1: for (int eta = HALF_W; eta < 30 - HALF_W; eta++) {
        VITIS_LOOP_532_2: for (int phi = 0; phi < 60; phi++) {
#pragma HLS PIPELINE II=1
...
# 562 "./calorimeter.h"
void calo_trigger_stream_ref(const tower_et_t towers[30][60],
                            tower_et_t seed_threshold,
                            cluster_et_t cluster_threshold,
                            TriggerObject trigger_objects[8],
                            ht_t *ht,
...
void calo_trigger_em_jet_stream_ref(const tower_et_t towers[30][60],
                                    ...
                                    TriggerObject em_objects[8],
                                    TriggerObject jet_objects[8],
                                    ht_t *ht,
                                    int *num_em_clusters,
                                    int *num_jet_clusters);

void produce_cluster_stream_3x3_iso5(const tower_et_t towers[30][60],
                            tower_et_t seed_threshold,
                            cluster_et_t cluster_threshold,
                            hls::stream<Cluster> &cluster_stream);

void produce_cluster_stream_7x7(const tower_et_t towers[30][60],
                            tower_et_t seed_threshold,
                            cluster_et_t cluster_threshold,
                            hls::stream<Cluster> &cluster_stream);


void produce_em_cluster_stream(const tower_et_t towers[30][60],
                               tower_et_t seed_threshold,
                               cluster_et_t cluster_threshold,
                               hls::stream<Cluster> &cluster_stream);

void produce_jet_cluster_stream(const tower_et_t towers[30][60],
                                tower_et_t seed_threshold,
                                cluster_et_t cluster_threshold,
                                hls::stream<Cluster> &cluster_stream);


void cluster_sort_build_stream(const tower_et_t towers[30][60],
                            tower_et_t seed_threshold,
                            cluster_et_t cluster_threshold,
                            TriggerObject trigger_objects[8],
                            int *num_clusters);

void cluster_sort_build_em_jet_stream(const tower_et_t towers[30][60],
                                      tower_et_t em_seed_threshold,
                                      cluster_et_t em_cluster_threshold,
                                      tower_et_t jet_seed_threshold,
                                      cluster_et_t jet_cluster_threshold,
...
__attribute__((sdx_kernel("calo_trigger_tower_stream_ref", 0))) void calo_trigger_tower_stream_ref(
    hls::stream<tower_et_t> &tower_in,
    tower_et_t em_seed_threshold,
    cluster_et_t em_cluster_threshold,
    tower_et_t jet_seed_threshold,
...
#pragma HLS ARRAY_PARTITION variable=top_clusters complete dim=1
 int local_num_clusters = 0;
    ...
    VITIS_LOOP_668_1: for (int i = 0; i < NUM_CANDIDATES; i++){
#pragma HLS PIPELINE II=1
...
#pragma HLS INLINE
 ...
 if (!candidate.valid) {
    ...
    INSERT_POSITION: for (int j = 0; j < 8; j++) {
        if (!top_clusters[j].valid || cluster_is_better(candidate, top_clusters[j])) {

            VITIS_LOOP_113_1: for (int k = 8 - 1; k > j; k--) {
...
#pragma HLS ARRAY_PARTITION variable=top_clusters complete dim=1
 ...
 clear_top_clusters(top_clusters);
    ...
    FIND_CLUSTER: for (int i = 0; i < (30 * 60); ++i) {
#pragma HLS PIPELINE II=1
...
ht_t compute_ht(const tower_et_t towers[30][60]) {
    ht_t ht = 0;
    HT_ETA: for (int eta = 0; eta < 30; eta++) {
        HT_PHI: for (int phi = 0; phi < 60; phi++) {
#pragma HLS PIPELINE II=1
...
#pragma HLS ARRAY_PARTITION variable=top_clusters complete dim=1
 VITIS_LOOP_159_1: for (int i = 0; i < 8; ++i) {
#pragma HLS UNROLL
...
__attribute__((sdx_kernel("calo_trigger_tower_stream_ref", 0))) void calo_trigger_tower_stream_ref(
    ...
    int *num_jet_clusters)
{
#line 33 "/scratch/emettner/tachep-fpga-project-26/run_calorimeter_towerStreaming.tcl"
#pragma HLSDIRECTIVE TOP name=calo_trigger_tower_stream_ref
# 278 "calorimeter.cpp"
                               ...
                               hls::stream<Cluster> &cluster_stream)
{
    ...
    VITIS_LOOP_328_1: for (int eta = R; eta < 30 - R; eta++) {
        VITIS_LOOP_329_2: for (int phi = 0; phi < 60; phi++) {
#pragma HLS PIPELINE II=1

 Cluster candidate;
            init_cluster(candidate, eta, phi, 3, 5);

            tower_et_t seed_et = towers[eta][phi];

            if (seed_et >= seed_threshold &&
                is_local_maximum<3>(towers, eta, phi)) {
                ...
                if (core3_et >= cluster_threshold && is_em_like(core3_et, iso5_et, ring7_et)) {
                    set_valid_cluster(candidate, core3_et, iso5_et, eta, phi, 3, 5);
                }
            }
                                ...
                                hls::stream<Cluster> &cluster_stream)
{
    ...
    VITIS_LOOP_364_1: for (int eta = R; eta < 30 - R; eta++) {
        VITIS_LOOP_365_2: for (int phi = 0; phi < 60; phi++) {
#pragma HLS PIPELINE II=1

 Cluster candidate;
            init_cluster(candidate, eta, phi, 7, 7);

            tower_et_t seed_et = towers[eta][phi];

            if (seed_et >= seed_threshold &&
                is_local_maximum<7>(towers, eta, phi)) {
                ...
                if (sum7_et >= cluster_threshold &&
                    ...
                    set_valid_cluster(candidate, sum7_et, ring7_et, eta, phi, 7, 7);
                }
            }
...
#pragma HLS ARRAY_PARTITION variable=top_clusters complete dim=1
 ...
 produce_cluster_stream_3x3_iso5(towers, seed_threshold, cluster_threshold, cluster_stream);
    select_top_n_from_stream_fixed<((30 - 2 * (7 / 2)) * 60)>(cluster_stream, top_clusters, num_clusters);
    build_trigger_objects(top_clusters, trigger_objects);
}
...
#pragma HLS STREAM variable=jet_cluster_stream depth=64
 ...
 Cluster em_top_clusters[8];
    Cluster jet_top_clusters[8];
...
#pragma HLS ARRAY_PARTITION variable=jet_top_clusters complete dim=1
 ...
 produce_em_cluster_stream(towers,
    ...
    select_top_n_from_stream_fixed<((30 - 2 * (7 / 2)) * 60)>(em_cluster_stream, em_top_clusters, num_em_clusters);
    select_top_n_from_stream_fixed<((30 - 2 * (7 / 2)) * 60)>(jet_cluster_stream, jet_top_clusters, num_jet_clusters);
...
void produce_em_jet_cluster_stream_from_towers(
    ...
    ht_t *ht)
{
    tower_et_t linebuf[7][60];
#pragma HLS ARRAY_PARTITION variable=linebuf complete dim=1

 tower_et_t window[7][7];
#pragma HLS ARRAY_PARTITION variable=window complete dim=0

 ht_t local_ht = 0;

    VITIS_LOOP_481_1: for (int eta = 0; eta < 30; eta++) {

        int write_row = eta % 7;


        VITIS_LOOP_486_2: for (int phi = 0; phi < 60; phi++) {
#pragma HLS PIPELINE II=1
 ...
 tower_et_t incoming = tower_in.read();
        ...
        if (eta >= 7 - 1) {
            ...
            int center_eta = eta - (7 / 2);

            VITIS_LOOP_500_3: for (int center_phi = 0; center_phi < 60; center_phi++) {
#pragma HLS PIPELINE II=1



 VITIS_LOOP_505_4: for (int d_eta = -(7 / 2);
                     d_eta <= (7 / 2);
                     d_eta++) {
#pragma HLS UNROLL

 VITIS_LOOP_510_5: for (int d_phi = -(7 / 2);
                                        d_phi <= (7 / 2);
                                        d_phi++) {
#pragma HLS UNROLL

 int src_eta = center_eta + d_eta;
                        int src_row = src_eta % 7;
...
#pragma HLS ARRAY_PARTITION variable=jet_top_clusters complete dim=1
 ...
 produce_em_jet_cluster_stream_from_towers(tower_in,
    ...
    select_top_n_from_stream_fixed<((30 - 2 * (7 / 2)) * 60)>(em_cluster_stream,
                                                            em_top_clusters,
                                                            num_em_clusters);

    select_top_n_from_stream_fixed<((30 - 2 * (7 / 2)) * 60)>(jet_cluster_stream,
                                                            jet_top_clusters,
                                                            num_jet_clusters);
