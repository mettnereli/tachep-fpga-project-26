...
void find_clusters_stream(const tower_et_t towers[30][60],
    ...
    VITIS_LOOP_288_1: for (int eta = HALF_W; eta < 30 - HALF_W; eta++) {
        VITIS_LOOP_289_2: for (int phi = 0; phi < 60; phi++) {
#pragma HLS PIPELINE II=1
...
# 327 "./calorimeter.h"
__attribute__((sdx_kernel("calo_trigger_stream_ref", 0))) void calo_trigger_stream_ref(const tower_et_t towers[30][60],
                            tower_et_t seed_threshold,
                            cluster_et_t cluster_threshold,
                            TriggerObject trigger_objects[8],
                            ht_t *ht,
...
#pragma HLSDIRECTIVE TOP name=calo_trigger_stream_ref
# 253 "calorimeter.cpp"
...
#pragma HLS ARRAY_PARTITION variable=top_clusters complete dim=1
 int local_num_clusters = 0;
    ...
    VITIS_LOOP_288_1: for (int i = 0; i < 8; i++) {
#pragma HLS UNROLL
 top_clusters[i].et = 0;
        top_clusters[i].isolation_et = 0;
        top_clusters[i].eta = 0;
    ...
    VITIS_LOOP_299_2: for (int i = 0; i < ((30 - 2 * (5 / 2)) * 60); i++) {
#pragma HLS PIPELINE II=1
